...
export async function GET() {
  ...
  const letters = code
    ...
    .map((letter, index) => {
      ...
      return `<text x="${x}" y="${y}" transform="rotate(${rotations[index]} ${x} ${y})">${letter}</text>`;
    })
    .join("");
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="190" height="64" viewBox="0 0 190 64">
    ...
    <g font-family="Arial, sans-serif" font-size="26" font-weight="600" fill="#2c1119" letter-spacing="2">${letters}</g>
    <text x="178" y="59" text-anchor="end" font-family="monospace" font-size="6" fill="#431c26" fill-opacity=".28">${seed.slice(0, 6)}</text>
  </svg>`;
  const image = await sharp(Buffer.from(svg)).png().toBuffer();

  const response = new NextResponse(new Uint8Array(image), {
    headers: {
