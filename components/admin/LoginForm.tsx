  ...
  return (
    <form action={action} className="mt-10 space-y-6">
      <label className="block">
        ...
        <input name="login" type="text" autoComplete="username" required className="h-13 w-full border border-ink/15 bg-transparent px-4 text-sm outline-none transition-colors focus:border-wine" />
      </label>
      <label className="block">
        <span className="mb-2 block text-[10px] uppercase tracking-[0.09em] text-ink/45">Пароль</span>
        <input name="password" type="password" autoComplete="current-password" required className="h-13 w-full border border-ink/15 bg-transparent px-4 text-sm outline-none transition-colors focus:border-wine" />
      </label>
      <div>
        ...
        <div className="grid grid-cols-[minmax(0,1fr)_190px_44px] gap-2">
          <input name="captcha" inputMode="text" autoComplete="off" maxLength={5} required aria-label="Код с изображения" className="h-16 min-w-0 border border-ink/15 bg-transparent px-4 text-center text-lg uppercase tracking-[0.18em] outline-none focus:border-wine" />
